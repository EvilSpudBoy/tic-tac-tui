...
\author{Project: \texttt{tic-tac-tui}}
...
\texttt{--strict-repetition} & Disallow repetition by a matching board plus active-grid state. \\
\end{longtable}
...
\item \textbf{Shift}: move the active 3x3 grid one step up, down, left, or right (when legal).
\end{itemize}

\subsection{Repetition handling}
\begin{itemize}
\item Default mode avoids immediate move-level backtracking loops.
\item \texttt{--strict-repetition} enforces no repeated full game state, including grid location.
\end{itemize}
...
\item For strict anti-loop behavior, add \texttt{--strict-repetition}.
